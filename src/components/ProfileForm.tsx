  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 rtl">
      ...
      <div className="space-y-6">
        <div className="group">
          <Label htmlFor="name" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"><User className="w-4 h-4 text-slate-400" />نام</Label>
          <Input id="name" {...register('name')} className="w-full h-12 px-4 bg-slate-50/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl text-right" placeholder="نام خود را وارد کنید" />
        ...
        <div className="group">
          ...
          <Input id="jobName" {...register('jobName')} className="w-full h-12 px-4 bg-slate-50/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl text-right" placeholder="شغل خود را وارد کنید" />
          {errors.jobName && <p className="text-red-500 text-xs mt-2">{errors.jobName.message}</p>}
        </div>
        <div className="group">
          <Label htmlFor="bio" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"><FileText className="w-4 h-4 text-slate-400" />بیوگرافی</Label>
          <Textarea id="bio" {...register('bio')} rows={4} className="w-full px-4 py-3 bg-slate-50/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl text-right resize-none" placeholder="درباره خودتان بنویسید..." />
        ...
        <div className={cn('grid transition-all duration-300 ease-out', isPasswordSectionOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0')}>
          <div className="overflow-hidden">
            <div className="p-5 space-y-5">
              <div className="group">
                <Label htmlFor="currentPassword" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"><KeyRound className="w-4 h-4 text-amber-500" />رمز عبور فعلی</Label>
                <div className="relative">
                  <Input id="currentPassword" {...register('currentPassword')} type={showCurrentPassword ? 'text' : 'password'} className="w-full h-12 px-4 pl-12 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-right" placeholder="رمز عبور فعلی" />
                  <button type="button" onClick={() => setShowCurrentPassword(!showCurrentPassword)} className="absolute left-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                    {showCurrentPassword ? <EyeOff className="w-4 h-4 text-slate-400" /> : <Eye className="w-4 h-4 text-slate-400" />}
                  </button>
                </div>
                {errors.currentPassword && <p className="text-red-500 text-xs mt-2">{errors.currentPassword.message}</p>}
              </div>
              <div className="group">
                <Label htmlFor="newPassword" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"><Lock className="w-4 h-4 text-amber-500" />رمز عبور جدید</Label>
                <div className="relative">
                  <Input id="newPassword" {...register('newPassword')} type={showNewPassword ? 'text' : 'password'} className="w-full h-12 px-4 pl-12 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-right" placeholder="رمز عبور جدید" />
                  <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute left-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
              ...
              <div className="group">
                <Label htmlFor="confirmNewPassword" className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"><Check className="w-4 h-4 text-amber-500" />تکرار رمز عبور جدید</Label>
                <div className="relative">
                  <Input id="confirmNewPassword" {...register('confirmNewPassword')} type={showConfirmPassword ? 'text' : 'password'} className="w-full h-12 px-4 pl-12 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-right" placeholder="تکرار رمز عبور جدید" />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute left-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
