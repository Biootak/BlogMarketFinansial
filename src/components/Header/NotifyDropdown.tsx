...
const NotifyDropdown = memo(({ className = "hidden sm:block" }: Props) => {
	...
	const NotificationItem = useCallback(
		({ item }: { item: Solution }) => (
			...
			>
				<Avatar sizeClass="w-8 h-8 sm:w-12 sm:h-12" radius="rounded-full" />
						...
						>
							<Popover.Panel className="absolute z-10 w-screen max-w-xs sm:max-w-sm px-4 mt-3 -end-28 sm:end-0 sm:px-0">
								<div className="overflow-hidden rounded-2xl shadow-lg ring-1 ring-black ring-opacity-5">
									<div
										className={`relative grid gap-8 bg-white dark:bg-neutral-800 p-7 ${isDarkMode ? "dark" : ""}`}
									>
										<h3 className="text-xl font-semibold">اعلان‌ها</h3>
										<ul>
											{solutions.map((item) => (
												<NotificationItem
													key={`${idPrefix}-${item.name}`}
													item={item}
												/>
											))}
										</ul>
