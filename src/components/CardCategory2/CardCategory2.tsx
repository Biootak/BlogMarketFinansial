...
const CardCategory2: FC<CardCategory2Props> = ({ className = '', taxonomy, index }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${colorStyle.bg}`}
        />

        {index && (
          <div
            className={`absolute top-3 end-3 z-10 w-7 h-7 rounded-full ${colorStyle.bg} ${colorStyle.text} flex items-center justify-center text-xs font-bold shadow-sm ring-2 ${colorStyle.ring}`}
          >
            {index}
          </div>
        )}
        ...
        <div className="relative z-10">
          <div
            className={`absolute -inset-2 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-500 blur-xl ${colorStyle.bg}`}
          />
          <motion.div
            className="relative"
          ...
          >
            <div
              className={
                'absolute -inset-1 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300'
              }
            />
            <NcImage
        ...
        <div
          className={
            'absolute bottom-0 start-0 end-0 h-1 bg-gradient-to-r from-transparent via-primary-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300'
          }
        />
      </motion.div>
    </Link>
