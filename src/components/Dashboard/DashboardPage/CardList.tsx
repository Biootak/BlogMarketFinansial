  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none overflow-hidden rounded-xl sm:rounded-2xl lg:rounded-3xl">
          <div
            ...
            {canEditPost && (
              <div>
                <DropdownMenu dir="rtl">
                  <DropdownMenuTrigger className="focus:outline-none" aria-label="گزینه‌های بیشتر">
                    <motion.div
                  ...
                  <DropdownMenuContent
                    align="start"
                    className="rounded-xl border-slate-200 dark:border-slate-700 shadow-xl min-w-[160px] sm:min-w-[180px]"
                  >
                    {(session?.user?.role === 'ADMIN' ||
                      session?.user?.role === 'SUPER_ADMIN' ||
                ...
                <button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(
                      window.location.origin + getPostLink(postType, slug),
                    );
                    toast({
                      title: 'کپی شد!',
        ...
        <div className="relative p-3 sm:p-3.5 md:p-4 lg:p-5 flex flex-col flex-grow">
          {/* Meta info */}
          {!hiddenAuthor ? (
            <div className="mb-2 sm:mb-2.5">{postMeta}</div>
          ) : (
