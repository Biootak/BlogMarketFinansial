...
const META: [string, Omit<Meta, 'header'>][] = [
  [
    '/observability',
    {
      label: 'مرکز مشاهده‌پذیری',
      family: 'observe',
      description:
        'سلامت سرویس‌ها، تأخیر، خطا و رد ممیزی، هر عدد مستقیم از SystemLog و AuditLog خوانده می‌شود.',
      icon: Radar,
      sibling: '/dashboard/observability/services',
      siblingLabel: 'سرویس‌ها',
    },
  ],
  [
    '/service-requests',
    {
      ...
      family: 'operations',
      description: 'صف درخواست‌های واقعی کاربران، با تمرکز روی وضعیت و اقدام بعدی.',
      icon: LifeBuoy,
      sibling: '/dashboard/my-requests',
      siblingLabel: 'درخواست‌های من',
    },
  ],
  [
    '/wallet',
    {
      label: 'کیف پول',
      family: 'money',
      description: 'موجودی‌ها و حرکت‌های مالی حساب.',
      icon: WalletCards,
      sibling: '/dashboard/transfer',
      siblingLabel: 'انتقال وجه',
    },
  ],
  [
    '/settings',
    {
      label: 'تنظیمات',
      family: 'settings',
      description: 'تنظیمات این پورتال، بدون خروج از جریان کاری.',
      icon: Settings2,
      sibling: '/dashboard/edit-profile',
      siblingLabel: 'پروفایل',
    },
  ],
  [
    '/security',
    {
      label: 'امنیت',
      family: 'security',
      description: 'دستگاه‌ها، نشست‌ها و کنترل‌های امنیتی.',
      icon: LockKeyhole,
      sibling: '/dashboard/devices',
      siblingLabel: 'دستگاه‌ها',
    },
  ],
  [
    '/permissions',
    {
      label: 'دسترسی‌ها',
      family: 'security',
      description: 'کنترل نقش‌ها و سطح دسترسی در پلتفرم.',
      icon: ShieldCheck,
      sibling: '/dashboard/roles',
      siblingLabel: 'نقش‌ها',
    },
  ],
  [
    '/users',
    {
      label: 'کاربران',
      family: 'people',
      description: 'مدیریت دسترسی و وضعیت کاربران پلتفرم.',
      icon: Landmark,
      sibling: '/dashboard/roles',
      siblingLabel: 'نقش‌ها',
    },
...
export function RouteFrame({ children }: { children: ReactNode }) {
  ...
  const base = pathname.startsWith('/customer')
    ? '/customer/dashboard'
  ...
  return (
    <div className="route-frame" data-route-family={meta.family} data-route-path={pathname}>
      <div className="route-frame__utility" aria-label="زمینه مسیر">
        ...
        {!isHome && (
          <Link href={base} className="route-frame__back">
            <ArrowLeft aria-hidden size={14} /> بازگشت
          </Link>
        )}
      ...
      <header className="route-frame__header">
        <div className="route-frame__mark" aria-hidden>
          <Icon size={21} strokeWidth={1.7} />
        </div>
        <div className="route-frame__heading">
          <span className="route-frame__eyebrow">مرکز عملیات</span>
        ...
        <div className="route-frame__tools">
          <span className="route-frame__state">
            <span className="route-frame__state-dot" aria-hidden /> داده زنده
          </span>
          {meta.sibling && (
            <Link href={meta.sibling} className="route-frame__sibling">
              {meta.siblingLabel}
              <ArrowUpLeft aria-hidden size={14} />
            </Link>
          )}
        </div>
      </header>
