...
export const EmbedRender = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    if (provider === 'youtube' || provider === 'vimeo') {
      return [
        ...
        mergeAttributes(HTMLAttributes, {
          'data-embed': '',
          style: 'margin: 1rem 0; position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; border-radius: 0.5rem;',
        }),
        [
          'iframe',
          {
    ...
    return [
      ...
      mergeAttributes(HTMLAttributes, {
        'data-embed': '',
        style: 'margin: 1rem 0; padding: 1rem; background-color: #f3f4f6; border-radius: 0.5rem; text-align: center;',
      }),
