    ...
    if (input.intent === '2fa') {
      ...
      if (!totpOk && !backupCodeId) {
        ...
        return {
          success: false,
          error: decryptFailed
            ? // 2026-08-14: decrypt شکست خورده یعنی سکرت TOTP کاربر در سمت سرور
              // قابل رمزگشایی نیست — مشکل داخلی است، نه کد اشتباه کاربر. کد
              // داخلی (TOTP-DEC) به کاربر نشان داده نمی‌شود؛ جزئیات واقعی در
              // serverLog بالا ثبت شده. مسیر کد پشتیبان (مستقل از AUTH_SECRET)
              // همچنان باز است — کاربر را به آن راهنمایی می‌کنیم.
              'ورود با کد تأیید موقتاً ممکن نیست؛ لطفاً از «ورود با کد پشتیبان» استفاده کنید یا با پشتیبانی تماس بگیرید'
