...
import '@/app/dashboard/dashboard-cockpit-layout.css';
...
export default async function ExchangeLayout({ children }: { children: React.ReactNode }) {
  ...
  if (PLATFORM_ADMINS.has(role)) {
    ...
    return <SiteSettingsProvider initialSettings={initialSettings}><Suspense fallback={null}><DashboardProviders userRole={role} portal="exchange" staffRole="OWNER">{children}</DashboardProviders></Suspense></SiteSettingsProvider>;
  }
  const membership = await getExchangeForUser();
  if (!membership) redirect('/');
  if (membership.exchange.status === 'SUSPENDED') redirect('/exchange-suspended');
  return <SiteSettingsProvider initialSettings={initialSettings}><Suspense fallback={null}><DashboardProviders userRole={role} portal="exchange" staffRole={membership.staffRole}>{children}</DashboardProviders></Suspense></SiteSettingsProvider>;
}
