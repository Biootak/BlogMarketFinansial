...
const PageSearch = ({}) => {
  ...
  const posts = useMemo(() => DEMO_POSTS.filter((_, i) => i < 12), []);
  const cats = useMemo(() => DEMO_CATEGORIES.filter((_, i) => i < 15), []);
  const tags = useMemo(() => DEMO_CATEGORIES.filter((_, i) => i < 32), []);
  const authors = useMemo(() => DEMO_AUTHORS.filter((_, i) => i < 12), []);
  ...
  return (
    <div className={`nc-PageSearch`}>
      ...
      <div className="w-screen px-2 xl:max-w-screen-2xl mx-auto">
        {/* Use a static height for the image container to prevent layout shift */}
        <div className="rounded-3xl md:rounded-[40px] relative aspect-w-16 aspect-h-9 lg:aspect-h-5 overflow-hidden z-0 h-[400px] lg:h-[200px]">
          <NcImage
            alt="search"
            fill
            ...
            <header className="w-full max-w-3xl mx-auto text-center flex flex-col items-center">
              ...
              <div className="w-full text-sm text-start mt-4 text-neutral-500 dark:text-neutral-300">
                <div className="inline-block space-x-1.5 sm:space-x-2.5 rtl:space-x-reverse">
                  ...
                  {/* Use span instead of NcLink for non-interactive elements */}
                  <span className="inline-block font-normal">Design</span>
                  <span className="inline-block font-normal">Photo</span>
                  <span className="inline-block font-normal">Vector</span>
                  <span className="inline-block font-normal">Frontend</span>
                </div>
              </div>
            </header>
          </div>
      ...
      <div className="container py-16 lg:pb-28 lg:pt-20 space-y-16 lg:space-y-28">
        <main>
          ...
          {/* LOOP ITEMS */}
          {/* Use a single container and conditionally render content */}
          {/* <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 md:gap-8 mt-8 lg:mt-10">
            {tabActive === 'Articles' && posts.map((post) => <Card11 key={post.id} post={post} />)}
            {tabActive === 'Categories' &&
              cats.map((cat) => <CardCategory2 key={cat.id} taxonomy={cat} />)}
            {tabActive === 'Tags' && (
            ...
            {tabActive === 'Authors' &&
              authors.map((author) => <CardAuthorBox2 key={author.id} author={author} />)}
          </div> */}
