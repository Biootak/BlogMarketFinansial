    ...
    <div className={styles.page}>
      ...
      <nav className={styles.focusNav} aria-label="بخش‌های اصلی">
        {focusItems.map(({ icon: Icon, label, value, note }, index) => (
          <a href={index === 0 ? '#percentiles' : index === 1 ? '#traffic' : '#services'} key={label} className={styles.focusItem}><span className={styles.focusIndex}>۰{index + 1}</span><Icon size={17} aria-hidden /><span><strong>{label}</strong><small>{value} · {note}</small></span><ArrowUpLeft size={15} className={styles.focusArrow} aria-hidden /></a>
        ))}
      </nav>
