...
function MiniSparkline({
  ...
  const buckets = Array.from({ length: 7 }, (_, i) => {
    ...
    return customers.filter(
      (c) => filter(c) && new Date(c.createdAt).getTime() >= start && new Date(c.createdAt).getTime() < end,
    ).length;
  });
  ...
  return (
    ...
    >
      ...
      {pts[pts.length - 1] && (() => {
        ...
        return <circle cx={lx} cy={ly} r="2" fill="var(--spark-color, currentColor)" opacity="0.9" />;
      })()}
    </svg>
  );
}
  ...
  return (
    <svg width="30" height="18" viewBox="0 0 30 18" aria-hidden="true">
      ...
      <path
        d="M3,16 A12,12 0 0,1 27,16"
        fill="none"
        stroke={score >= 70 ? 'var(--nova-rose,#ef4444)' : score >= 40 ? 'var(--nova-amber,#f59e0b)' : 'var(--nova-emerald,#22c55e)'}
        strokeWidth="3"
        strokeLinecap="round"
  ...
  const handleAdd = useCallback(() => {
    ...
    startAddTransition(async () => {
      ...
      if (result.success) {
        ...
        setSheetMode('detail');
        setFormState({ fullName: '', fatherName: '', nationalId: '', phone: '', email: '', city: '', address: '', notes: '' });
        router.refresh();
      } else {
        toast({ title: 'خطا', description: result.error.message, variant: 'destructive' });
      }
    ...
    <div className={s.root}>
      ...
      <PageHeader
        ...
        actions={
          currentExchangeId ? (
            <Button
              size="sm"
              onClick={() => { setSheetMode('add'); setSelected(null); }}
            >
      ...
      <div className={s.kpiStrip} role="list" aria-label="آمار مشتریان">
        {(
          ...
          <div key={item.label} className={s.kpiCard} role="listitem">
            <div className={s.kpiTop}>
              <span className={s.kpiIcon} aria-hidden>{item.icon}</span>
              <MiniSparkline
                customers={customers}
      ...
      {customers.length === 0 ? (
        <EmptyState
          ...
          description={currentQuery ? 'جستجو نتیجه‌ای نداشت' : 'هنوز مشتری‌ای در این صرافی ثبت نشده'}
          action={
            currentExchangeId ? (
              <Button size="sm" onClick={() => { setSheetMode('add'); }}>
          ...
          <table className={s.table} aria-label="جدول مشتریان">
            <thead>
              <tr>
                ...
                <th>وضعیت</th>
                <th>تاریخ ثبت</th>
                <th><span className="sr-only">اقدامات</span></th>
              </tr>
            </thead>
            <tbody>
              {customers.map((c, i) => (
                <tr
                  ...
                  onClick={() => { setSelected(c); setSheetMode('detail'); }}
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter') { setSelected(c); setSheetMode('detail'); } }}
                ...
                >
                  ...
                  <td>
                    <div className={s.contactCell}>
                      <span dir="ltr" className={s.phone}>{c.phone}</span>
                      {c.email && <span dir="ltr" className={s.email}>{c.email}</span>}
                    </div>
                  </td>
                  ...
                  <td>
                    <div className={s.kycCell}>
                      ...
                      <span className={s.kycStatus}>{KYC_STATUS_FA[c.kycStatus] ?? c.kycStatus}</span>
                    </div>
                  </td>
            ...
            <>
              ...
              <div className={s.sheetBody}>
                ...
                <section className={s.sheetSection}>
                  ...
                  <dl className={s.dl}>
                    {selected.fatherName && <><dt>نام پدر</dt><dd>{selected.fatherName}</dd></>}
                    <dt>شماره تماس</dt><dd dir="ltr">{selected.phone}</dd>
                    {selected.email && <><dt>ایمیل</dt><dd dir="ltr">{selected.email}</dd></>}
                    {selected.city && <><dt>شهر</dt><dd>{selected.city}</dd></>}
                    {selected.address && <><dt>آدرس</dt><dd>{selected.address}</dd></>}
                    <dt>تاریخ ثبت</dt><dd>{formatDate(selected.createdAt)}</dd>
                  </dl>
                </section>
                ...
                <section className={s.sheetSection}>
                  ...
                  <dl className={s.dl}>
                    ...
                    <dt>وضعیت</dt><dd>{KYC_STATUS_FA[selected.kycStatus] ?? selected.kycStatus}</dd>
                  </dl>
                </section>
                ...
                <section className={s.sheetSection}>
                  ...
                  <dl className={s.dl}>
                    <dt>سقف شخصی</dt><dd>{fmtLimit(selected.personalLimitAf)}</dd>
                  </dl>
                </section>
      ...
      <Sheet
        open={sheetMode === 'add'}
        onOpenChange={(o) => { if (!o) setSheetMode('detail'); }}
      >
        <SheetContent side="left" className={s.detailSheet} dir="rtl">
          <SheetHeader className={s.sheetHeader}>
          ...
          <div className={s.sheetBody}>
            <div className={s.addForm}>
              {[
                { id: 'fullName', label: 'نام و نام خانوادگی *', key: 'fullName' as const, required: true },
                { id: 'fatherName', label: 'نام پدر', key: 'fatherName' as const },
                { id: 'phone', label: 'شماره تماس *', key: 'phone' as const, required: true, dir: 'ltr' },
                { id: 'nationalId', label: 'کد ملی / شناسنامه', key: 'nationalId' as const, dir: 'ltr' },
              ...
              >
                {addPending ? <span className={s.spinner} aria-label="در حال ذخیره" /> : <Plus size={14} aria-hidden />}
                ذخیره مشتری
              </Button>
            </div>
          </div>
