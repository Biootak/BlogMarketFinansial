...
import { createRateList, deleteRateList, getRateLists, updateRateList } from '@/actions/rate-lists';
import {
  ActionButton,
  DashboardPageHeader,
...
const RateListsPage = () => {
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/40 p-3 sm:p-5 lg:p-8 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950/30"
      dir="rtl"
    >
      <div className="mx-auto max-w-7xl space-y-6">
        <DashboardPageHeader
          title="مدیریت لیست‌های نرخ"
          description="مشاهده و مدیریت لیست‌های نرخ ارز"
        >
          <DashboardSearchInput
            value={filterOptions.search}
            onChange={(v) => setFilterOptions((p) => ({ ...p, search: v }))}
            placeholder="جستجو..."
          />
          <FilterSelect
            value={filterOptions.currency}
            onChange={(v) => setFilterOptions((p) => ({ ...p, currency: v }))}
            options={currencyOptions}
          />
          <FilterSelect
            value={filterOptions.dateRange}
            onChange={(v) => setFilterOptions((p) => ({ ...p, dateRange: v }))}
            options={dateOptions}
          />
          <FilterSelect
            value={filterOptions.sortBy}
            onChange={(v) => setFilterOptions((p) => ({ ...p, sortBy: v }))}
            options={sortOptions}
          />
        ...
        {filteredRateLists.length === 0 ? (
          <div className="rounded-2xl border border-slate-200/60 bg-white/80 p-12 text-center shadow-sm backdrop-blur-sm dark:border-slate-700/50 dark:bg-slate-800/80">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              هنوز هیچ لیست نرخی در سیستم ثبت نشده است.
            </p>
          </div>
        ) : (
      ...
      >
        <DialogContent
          className="max-h-[95vh] sm:max-h-[90vh] w-[calc(100%-1rem)] max-w-2xl overflow-hidden rounded-3xl border border-slate-200/60 bg-white/95 p-0 shadow-2xl backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/95"
          dir="rtl"
        >
          ...
          <form
            onSubmit={handleSubmit(editingRateList ? handleEditSubmit : handleCreateSubmit)}
            className="max-h-[calc(95vh-120px)] sm:max-h-[calc(90vh-140px)] overflow-y-auto p-5 sm:p-7"
          >
                ...
                <ScrollArea className="h-72 rounded-2xl border border-slate-200/60 bg-gradient-to-br from-slate-50/80 to-slate-100/50 p-4 shadow-inner dark:border-slate-700/50 dark:from-slate-900/80 dark:to-slate-800/50">
                  <div className="space-y-2.5">
                    {fields.map((field, index) => (
                      <div
                        key={field.id}
                        className="group flex items-center gap-2.5 rounded-xl border border-slate-200/60 bg-white/90 p-3 shadow-sm backdrop-blur-sm transition-all duration-200 hover:shadow-md dark:border-slate-700/50 dark:bg-slate-800/90"
                      >
                        <Input
