...
export default async function AccountPage() {
  ...
  const secScore =
    (dbUser?.emailVerified ? 40 : 0) + (dbUser?.twoFactorEnabled ? 40 : 0) + 20; /* ثبت‌نام = ۲۰ */
    ...
    <div className={s.root} dir="rtl">
      ...
      <section className={s.hero} aria-label="هویت کاربر">
        ...
        >
          ...
          <path
            d="M 360 -30 A 120 120 0 0 1 480 90"
            className={s.bgArc}
            strokeWidth="0.6"
            fill="none"
          />
          <path
            d="M -20 60  A 100 100 0 0 1 80 160"
            className={s.bgArc}
            strokeWidth="0.6"
            fill="none"
          />
        ...
        <div className={s.heroInner}>
          ...
          <div className={s.secGauge} aria-label={`امتیاز امنیت ${secScore} از ۱۰۰`}>
            <svg viewBox="0 0 36 36" className={s.secSvg} aria-hidden>
              <circle cx="18" cy="18" r="16" fill="none" strokeWidth="2" className={s.secTrack} />
              <circle
                cx="18"
                cy="18"
                r="16"
      ...
      <div className={s.kpiRow} role="list" aria-label="آمار کلی">
        {[
          ...
          },
          {
            ...
            value: dbUser?.emailVerified ? 'تأیید ✓' : 'تأیید نشده',
            label: 'ایمیل',
            color: 'emerald',
          },
          ...
          <div
            ...
            style={
              {
                '--kpi-c': `var(--nova-${color === 'brand' ? 'primary' : color})`,
      ...
      <div className={s.twoUp}>
        ...
        <section className={s.panel} aria-labelledby="sec-heading">
          <header className={s.panelHead}>
            ...
            <h2 id="sec-heading" className={s.panelTitle}>
              امنیت حساب
            </h2>
          </header>
          ...
          <div className={s.scoreBar}>
            ...
            <span className={s.scoreBarVal} style={{ color: secColor }}>
              {secScore}/۱۰۰
            </span>
        ...
        <section className={s.panel} aria-labelledby="detail-heading">
          <header className={s.panelHead}>
            ...
            <h2 id="detail-heading" className={s.panelTitle}>
              اطلاعات حساب
            </h2>
          </header>
      ...
      {devices.length > 0 && (
        <section className={s.panel} aria-labelledby="dev-heading">
          <header className={s.panelHead}>
            ...
            <h2 id="dev-heading" className={s.panelTitle}>
              آخرین دستگاه‌های متصل
            </h2>
            <a href="/dashboard/devices" className={s.panelLink}>
              مشاهده همه
            </a>
          </header>
          <ul className={s.deviceList} aria-label="دستگاه‌ها">
            {devices.map((dev, i) => (
              <li key={dev.id} className={s.deviceItem} style={{ '--i': i } as React.CSSProperties}>
                ...
                <span className={s.deviceBody}>
                  ...
                  <span className={s.deviceIp} dir="ltr">
                    {dev.ip ?? '—'}
                  </span>
                </span>
                ...
                <span
                  className={`${s.deviceBadge} ${dev.status === 'TRUSTED' ? s.deviceOk : s.deviceWarn}`}
                >
      ...
      {logs.length > 0 && (
        <section className={s.panel} aria-labelledby="log-heading">
          <header className={s.panelHead}>
            ...
            <h2 id="log-heading" className={s.panelTitle}>
              آخرین فعالیت‌ها
            </h2>
          ...
          <ol className={s.timeline} aria-label="تاریخچه فعالیت">
            {logs.map((log, i) => (
              <li key={log.id} className={s.tlItem} style={{ '--i': i } as React.CSSProperties}>
                ...
                <span className={s.tlContent}>
                  ...
                  {log.entityType && <span className={s.tlEntity}>{log.entityType}</span>}
