...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!assertSameOrigin(request)) {
      return NextResponse.json({ success: false, error: { code: 'CSRF', message: 'درخواست نامعتبر' } }, { status: 403 });
    }

    // چک احراز هویت - فقط ادمین‌ها می‌تونن cache رو پاک کنن
    const session = await auth();
    if (!session?.user) {
      return NextResponse.json({ success: false, error: { code: 'UNAUTHENTICATED', message: 'احراز هویت الزامی است' } }, { status: 401 });
    }
    ...
    if (!rl.success) {
      return NextResponse.json({ success: false, error: { code: 'RATE_LIMITED', message: 'تعداد درخواست بیش از حد مجاز است' } }, { status: 429 });
    }

    const { path } = await request.json();

    if (!path || typeof path !== 'string') {
      return NextResponse.json({ success: false, error: { code: 'BAD_REQUEST', message: 'مسیر نامعتبر است' } }, { status: 400 });
    }
    ...
    if (!isAllowed && !path.startsWith('/dashboard')) {
      return NextResponse.json({ success: false, error: { code: 'BAD_REQUEST', message: 'مسیر مجاز نیست' } }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'خطا در پاکسازی کش' } }, { status: 500 });
  }
}
