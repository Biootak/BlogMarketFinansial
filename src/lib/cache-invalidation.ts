...
/**
 * cache-invalidation — منطق خام پاک‌سازی کش (سه لایه)
 *
 * SECURITY-fix (2026-08-22): این توابع عمداً در `src/lib/` هستند نه در یک
 * فایل 'use server'. هر export در فایل 'use server' یک endpoint عمومی با
 * action-id قابل حدس در باندل کلاینت است؛ نسخهٔ بدون گارد روی چنین سطحی
 * یعنی هر کاربر ناشناس بتواند کش را پاک کند (cache-stampede / DoS).
 *
 * مصرف‌کنندگان معتبرِ سرور-به-سرور (logout، postActions، ...) این ماژول را
 * مستقیم import می‌کنند. فراخوانی از سمت کلاینت فقط از طریق
 * `@/actions/cacheActions` ممکن است که هر اکشنش گارد ادمین دارد.
 */
