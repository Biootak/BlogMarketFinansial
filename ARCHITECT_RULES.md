...
4. "فقط در صورت نیاز، خروجی را با tsc/lint بررسی می‌کنم؛ تست بصری با خود کاربر است."
...
در این موارد از گردش کار سنگین‌تر استفاده کن: جستجوی عمیق → آنالیز کوتاه → Edit → Verify.
...
## ۲. گردش کار پیش‌فرض — Build → Show → Improve
...
- بعد از تغییر کد: اگه کد TypeScript جدید نوشته شد → `npx tsc --noEmit`؛ برای تک‌های CSS/UI کوچک → صرفنظر.
