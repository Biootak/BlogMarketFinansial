...
// HasParentCategories applies the HasEdge predicate on the "parent_categories" edge.
func HasParentCategories() predicate.Category {
	return predicate.Category(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasParentCategoriesWith applies the HasEdge predicate on the "parent_categories" edge with a given conditions (other predicates).
func HasParentCategoriesWith(preds ...predicate.Category) predicate.Category {
	return predicate.Category(func(s *sql.Selector) {
		step := newParentCategoriesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
