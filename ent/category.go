...
type CategoryEdges struct {
	...
	ParentCategories []*Category `json:"parent_categories,omitempty"`
...
// ParentCategoriesOrErr returns the ParentCategories value or an error if the edge
// was not loaded in eager-loading.
func (e CategoryEdges) ParentCategoriesOrErr() ([]*Category, error) {
	if e.loadedTypes[1] {
		return e.ParentCategories, nil
	}
	return nil, &NotLoadedError{edge: "parent_categories"}
}
...
func (*Category) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case category.FieldID, category.FieldName, category.FieldSlug, category.FieldDescription, category.FieldThumbnail:
...
// QueryParentCategories queries the "parent_categories" edge of the Category entity.
func (c *Category) QueryParentCategories() *CategoryQuery {
	return NewCategoryClient(c.config).QueryParentCategories(c)
}
