...
type CategoryQuery struct {
	...
	withPosts            *PostQuery
	withParentCategories *CategoryQuery
...
// QueryParentCategories chains the current query on the "parent_categories" edge.
func (cq *CategoryQuery) QueryParentCategories() *CategoryQuery {
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withParentCategories: cq.withParentCategories.Clone(),
...
// WithParentCategories tells the query-builder to eager-load the nodes that are connected to
// the "parent_categories" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithParentCategories(opts ...func(*CategoryQuery)) *CategoryQuery {
	query := (&CategoryClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withParentCategories = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withParentCategories; query != nil {
		if err := cq.loadParentCategories(ctx, query, nodes,
			func(n *Category) { n.Edges.ParentCategories = []*Category{} },
			func(n *Category, e *Category) { n.Edges.ParentCategories = append(n.Edges.ParentCategories, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CategoryQuery) loadParentCategories(ctx context.Context, query *CategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Category)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Category)
	nids := make(map[string]map[*Category]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ParentCategoriesTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(category.ParentCategoriesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.ParentCategoriesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ParentCategoriesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "parent_categories" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CategoryQuery) loadChildCategories(ctx context.Context, query *CategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ChildCategoriesTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(category.ChildCategoriesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(category.ChildCategoriesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ChildCategoriesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "child_categories" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
