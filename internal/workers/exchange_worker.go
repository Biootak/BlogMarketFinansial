...
func (w *ExchangeRateWorker) runWithRetry() {
	...
	config := DefaultRetryConfig("ExchangeRateWorker", "FetchRates")
	
	err := RetryWithExponentialBackoff(config, func() error {
		return w.service.FetchRates(ctx)
	})
	...
	if err == nil {
		...
		duration := time.Since(startTime)
		log.Printf("[ExchangeRateWorker] Failed to fetch exchange rates (took %v): %v", duration, err)
	}
}
