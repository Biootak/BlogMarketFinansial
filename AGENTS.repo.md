...
- `src/lib/` — `db.ts` (Prisma singleton), `auth.ts`, `require-auth.ts`, `rate-limiter.ts`, `storage.ts` (S3/Liara), `revalidate.ts`, `exchange-rates.ts`, `tgju.ts`, `freeMarketRates.ts`, `safe-cache.ts`, `safe-fetch.ts`, `email/` (provider-neutral).
